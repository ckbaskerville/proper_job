...
class SettingsDialog:
    def __init__(self, parent, app_instance):
        ...
        # Load current data
        self.materials_data = copy.deepcopy(self.app.materials_dict)
        self.labour_data = copy.deepcopy(self.app.labour_cost_dict)
        self.runners_data = copy.deepcopy(self.app.runners_dict)
    ...
    def create_dialog(self):
        ...
        self.dialog.configure(bg=self.app.DarkTheme.BG_COLOR)
    ...
    def create_materials_tab(self):
        ...
        canvas = tk.Canvas(materials_frame, bg=self.app.DarkTheme.BG_COLOR, highlightthickness=0)
    ...
    def create_labour_tab(self):
        ...
        # Create canvas for scrolling
        canvas = tk.Canvas(labour_frame, bg=self.app.DarkTheme.BG_COLOR, highlightthickness=0)
        ...
        row = 0
        for material_type, hours in self.labour_data[0]["Carcass"].items():
        ...
        row = 0
        for material_type, hours in self.labour_data[0]["Drawers"].items():
        ...
        row = 0
        for material_type, costs in self.labour_data[0]["Face Frames"].items():
    ...
    def create_runners_tab(self):
        ...
        # Create canvas for scrolling
        canvas = tk.Canvas(runners_frame, bg=self.app.DarkTheme.BG_COLOR, highlightthickness=0)
    ...
    def refresh_doors_tree(self):
        ...
        # Add door types
        for door in self.labour_data[0]["Doors"]:
            values = (
                door["Material"],
                door["Type"],
                door["Per Door (hours)"],
    ...
    def add_door_type(self):
        ...
        if dialog.result:
            self.labour_data[0]["Doors"].append(dialog.result)
            self.refresh_doors_tree()
            self.has_changes = True
    ...
    def edit_door_type(self):
        ...
        door_data = self.labour_data[0]["Doors"][index]

        dialog = DoorTypeDialog(self.dialog, "Edit Door Type", door_data)
        if dialog.result:
            self.labour_data[0]["Doors"][index] = dialog.result
            self.refresh_doors_tree()
            self.has_changes = True
    ...
    def delete_door_type(self):
        ...
        if messagebox.askyesno("Confirm Delete", "Are you sure you want to delete this door type?"):
            index = self.doors_tree.index(selected[0])
            del self.labour_data[0]["Doors"][index]
            self.refresh_doors_tree()
            self.has_changes = True
    ...
    def save_all_changes(self):
        ...
        try:
            ...
            for material_type, var in self.carcass_labour_vars.items():
                self.labour_data[0]["Carcass"][material_type] = var.get()

            for material_type, var in self.drawers_labour_vars.items():
                self.labour_data[0]["Drawers"][material_type] = var.get()

            for material_type, vars_dict in self.face_frame_vars.items():
                self.labour_data[0]["Face Frames"][material_type]["Per Frame (hours)"] = vars_dict["per_frame"].get()
                self.labour_data[0]["Face Frames"][material_type]["Moulding"] = vars_dict["moulding"].get()

            # Save to files
            with open(self.app.MATERIALS_PATH, 'w') as f:
                json.dump([self.materials_data], f, indent=2)

            with open(self.app.LABOUR_PATH, 'w') as f:
                json.dump(self.labour_data, f, indent=2)

            with open(self.app.RUNNERS_PATH, 'w') as f:
                json.dump(self.runners_data, f, indent=2)

            # Update app instance data
            self.app.materials_dict = self.materials_data
            self.app.labour_cost_dict = self.labour_data[0]
            self.app.runners_dict = self.runners_data

            # Recreate calculator with new data
            self.app.calculator = self.app.QuoteCalculator(self.app.labour_cost_dict, self.app.materials_dict)

            # Update sheet prices in calculator
            for material in self.materials_data["Materials"]:
                for thickness in material["Cost"]:
                    self.app.calculator.set_sheet_price(
                        material["Material"],
                        thickness["Thickness"],
    ...
    def reset_all(self):
        ...
        if messagebox.askyesno("Confirm Reset", "Are you sure you want to reset all changes?"):
            ...
            # Reset labour vars
            for material_type, var in self.carcass_labour_vars.items():
                var.set(self.labour_data[0]["Carcass"][material_type])

            for material_type, var in self.drawers_labour_vars.items():
                var.set(self.labour_data[0]["Drawers"][material_type])

            for material_type, vars_dict in self.face_frame_vars.items():
                vars_dict["per_frame"].set(self.labour_data[0]["Face Frames"][material_type]["Per Frame (hours)"])
                vars_dict["moulding"].set(self.labour_data[0]["Face Frames"][material_type]["Moulding"])
...
class MaterialDialog:
    ...
    def __init__(self, parent, title, material_data=None):
        self.result = None
        ...
        self.material_name_var = tk.StringVar(value=material_data["Material"] if material_data else "")
        self.veneer_var = tk.BooleanVar(value=material_data["Veneer"] if material_data else False)
        self.hardwood_var = tk.BooleanVar(value=material_data["Hardwood"] if material_data else False)
        self.carcass_var = tk.BooleanVar(value=material_data["Carcass"] if material_data else False)
        self.door_var = tk.BooleanVar(value=material_data["Door"] if material_data else False)
        self.face_frame_var = tk.BooleanVar(value=material_data["Face Frame"] if material_data else False)

        self.create_ui()

        # Load existing cost data if editing
        if material_data and "Cost" in material_data:
            self.load_cost_data(material_data["Cost"])
    ...
    def create_ui(self):
        ...
        # Material name
        ttk.Label(main_frame, text="Material Name:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.material_name_var, width=30).grid(row=0, column=1, padx=5, pady=5)

        # Checkboxes
        ttk.Checkbutton(main_frame, text="Veneer", variable=self.veneer_var).grid(row=1, column=0, sticky="w", padx=5,
                                                                                  pady=2)
        ttk.Checkbutton(main_frame, text="Hardwood", variable=self.hardwood_var).grid(row=1, column=1, sticky="w",
                                                                                      padx=5, pady=2)
        ttk.Checkbutton(main_frame, text="Carcass", variable=self.carcass_var).grid(row=2, column=0, sticky="w", padx=5,
                                                                                    pady=2)
        ttk.Checkbutton(main_frame, text="Door", variable=self.door_var).grid(row=2, column=1, sticky="w", padx=5,
                                                                              pady=2)
        ttk.Checkbutton(main_frame, text="Face Frame", variable=self.face_frame_var).grid(row=3, column=0, sticky="w",
                                                                                          padx=5, pady=2)

        # Cost section
        cost_frame = ttk.LabelFrame(main_frame, text="Cost Information", padding=10)
        cost_frame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=5, pady=10)

        # Cost table
        columns = ('thickness', 'cost')
        self.cost_tree = ttk.Treeview(cost_frame, columns=columns, show='headings', height=6)

        self.cost_tree.heading('thickness', text='Thickness (mm)')
        self.cost_tree.heading('cost', text='Cost (exc. VAT)')

        self.cost_tree.column('thickness', width=150)
        self.cost_tree.column('cost', width=150)

        self.cost_tree.pack(fill=tk.BOTH, expand=True, pady=5)

        # Cost buttons
        cost_buttons = ttk.Frame(cost_frame)
        cost_buttons.pack(fill=tk.X, pady=5)

        ttk.Button(cost_buttons, text="Add Thickness", command=self.add_thickness).pack(side=tk.LEFT, padx=5)
        ttk.Button(cost_buttons, text="Edit Thickness", command=self.edit_thickness).pack(side=tk.LEFT, padx=5)
        ttk.Button(cost_buttons, text="Delete Thickness", command=self.delete_thickness).pack(side=tk.LEFT, padx=5)
    ...
    def load_cost_data(self, cost_data):
        """Load existing cost data"""
        self.cost_data = cost_data.copy()
        self.refresh_cost_tree()

    def refresh_cost_tree(self):
        """Refresh the cost tree"""
        for item in self.cost_tree.get_children():
            self.cost_tree.delete(item)

        for cost in self.cost_data:
            values = (cost["Thickness"], f"{cost['Sheet Cost (exc. VAT)']:.2f}")
            self.cost_tree.insert('', tk.END, values=values)

    def add_thickness(self):
        """Add thickness/cost entry"""
        dialog = ThicknessDialog(self.dialog, "Add Thickness")
        if dialog.result:
            self.cost_data.append(dialog.result)
            self.refresh_cost_tree()

    def edit_thickness(self):
        """Edit selected thickness"""
        selected = self.cost_tree.selection()
        if not selected:
            messagebox.showwarning("Selection Required", "Please select a thickness to edit.")
            return

        index = self.cost_tree.index(selected[0])
        thickness_data = self.cost_data[index]

        dialog = ThicknessDialog(self.dialog, "Edit Thickness", thickness_data)
        if dialog.result:
            self.cost_data[index] = dialog.result
            self.refresh_cost_tree()

    def delete_thickness(self):
        """Delete selected thickness"""
        selected = self.cost_tree.selection()
        if not selected:
            messagebox.showwarning("Selection Required", "Please select a thickness to delete.")
            return

        if messagebox.askyesno("Confirm Delete", "Are you sure you want to delete this thickness?"):
            index = self.cost_tree.index(selected[0])
            del self.cost_data[index]
            self.refresh_cost_tree()
    ...
    def ok_clicked(self):
        ...
        if not self.cost_data:
            messagebox.showerror("Error", "Please add at least one thickness/cost entry.")
            return

        self.result = {
            "Material": self.material_name_var.get().strip(),
...
class ThicknessDialog:
    """Dialog for adding/editing thickness and cost"""

    def __init__(self, parent, title, thickness_data=None):
        self.result = None
        self.dialog = tk.Toplevel(parent)
        self.dialog.title(title)
        ...
        self.thickness_var = tk.IntVar(value=thickness_data["Thickness"] if thickness_data else 18)
        self.cost_var = tk.DoubleVar(value=thickness_data["Sheet Cost (exc. VAT)"] if thickness_data else 0.0)
    ...
    def create_ui(self):
        ...
        ttk.Label(main_frame, text="Thickness (mm):").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.thickness_var, width=15).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Cost (exc. VAT):").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.cost_var, width=15).grid(row=1, column=1, padx=5, pady=5)
    ...
    def ok_clicked(self):
        self.result = {
            ...
            "Sheet Cost (exc. VAT)": self.cost_var.get()
        }
        self.dialog.destroy()
...
class DoorTypeDialog:
    """Dialog for adding/editing door types"""

    def __init__(self, parent, title, door_data=None):
        self.result = None
        self.dialog = tk.Toplevel(parent)
        self.dialog.title(title)
        ...
        self.material_var = tk.StringVar(value=door_data["Material"] if door_data else "")
        self.type_var = tk.StringVar(value=door_data["Type"] if door_data else "")
        self.per_door_var = tk.DoubleVar(value=door_data["Per Door (hours)"] if door_data else 0.0)
        self.moulding_var = tk.DoubleVar(value=door_data["Moulding"] if door_data else 0.0)
        self.cut_handle_var = tk.DoubleVar(value=door_data["Cut Handle"] if door_data else 0.0)
    ...
    def create_ui(self):
        ...
        ttk.Label(main_frame, text="Material:").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        material_combo = ttk.Combobox(main_frame, textvariable=self.material_var, width=20)
        material_combo['values'] = ["Sprayed MDF", "Hardwood", "Veneer", "Laminate", "Melamine", "MDF"]
        material_combo.grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Type:").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        type_combo = ttk.Combobox(main_frame, textvariable=self.type_var, width=20)
        type_combo['values'] = ["Shaker", "Flat"]
        type_combo.grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Per Door (hours):").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.per_door_var, width=20).grid(row=2, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Moulding (hours):").grid(row=3, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.moulding_var, width=20).grid(row=3, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Cut Handle (hours):").grid(row=4, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.cut_handle_var, width=20).grid(row=4, column=1, padx=5, pady=5)
...
class RunnerDialog:
    """Dialog for adding/editing runners"""

    def __init__(self, parent, title, runner_data=None):
        self.result = None
        self.dialog = tk.Toplevel(parent)
        self.dialog.title(title)
        ...
        self.length_var = tk.IntVar(value=runner_data["Length"] if runner_data else 300)
        self.capacity_var = tk.IntVar(value=runner_data["Capacity"] if runner_data else 40)
        self.price_var = tk.DoubleVar(value=runner_data["Price"] if runner_data else 0.0)
    ...
    def create_ui(self):
        ...
        ttk.Label(main_frame, text="Length (mm):").grid(row=0, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.length_var, width=15).grid(row=0, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Capacity (kg):").grid(row=1, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.capacity_var, width=15).grid(row=1, column=1, padx=5, pady=5)

        ttk.Label(main_frame, text="Price (£):").grid(row=2, column=0, sticky="w", padx=5, pady=5)
        ttk.Entry(main_frame, textvariable=self.price_var, width=15).grid(row=2, column=1, padx=5, pady=5)
